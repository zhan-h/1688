...
# 导入爬虫核心类
from data_1688.scraper import AlibabaScraperCore, FactoryScraperCore
from goofish.goofish import GoofishScraperCore
...
# ==================== 基础优化混入类 ====================
class OptimizedGUI:
    """为 GUI 提供批量插入、节流刷新等优化方法"""

    def __init__(self):
        self.insert_buffer = []  # 批量插入缓冲
        self.pending_json_update = False  # 节流标记
        self.last_percent_update = -1  # 上次更新的百分比整数部分
        self._batch_size = 20  # 每多少条批量插入一次

    def _flush_insert_buffer(self):
        """批量插入表格数据"""
        if not self.insert_buffer:
            return
        for data in self.insert_buffer:
            self._add_item_to_tree(data)
        self.insert_buffer.clear()
        self.tree.yview_moveto(1)

    def _throttled_json_update(self):
        """节流更新 JSON 视图"""
        self.pending_json_update = False
        self.update_json_view()

    def _schedule_json_update(self):
        """安排一次节流的 JSON 更新"""
        if not self.pending_json_update:
            self.pending_json_update = True
            self.after(500, self._throttled_json_update)

    def _batch_add_data(self, data):
        """添加数据到缓冲，并触发批量插入"""
        self.insert_buffer.append(data)
        if len(self.insert_buffer) >= self._batch_size:
            self.after(0, self._flush_insert_buffer)
        self._schedule_json_update()

    def _update_progress_with_throttle(self, current, total, percent):
        """仅在百分比整数部分变化时更新进度条"""
        int_percent = int(percent)
        if int_percent != self.last_percent_update:
            self.last_percent_update = int_percent
            self.segment_progress.set_value(percent)
            self.percent_label.config(text=f"{percent:.2f}%")

    def on_scrape_finished_cleanup(self):
        """采集结束时刷新所有缓冲"""
...
# ==================== 1688 采集 GUI ====================
class AlibabaScraperGUI(ttk.Frame, OptimizedGUI):
    def __init__(self, parent):
        ttk.Frame.__init__(self, parent)
        OptimizedGUI.__init__(self)
    ...
    def update_progress(self, current, total, page, async_num, items_count):
        ...
        self._update_progress_with_throttle(current, total, percent)
    ...
    def on_scrape_finished(self, total_items):
        self.is_scraping = False
        self.on_scrape_finished_cleanup()
...
# ==================== 闲鱼采集 GUI ====================
class GoofishScraperGUI(ttk.Frame, OptimizedGUI):
    def __init__(self, parent):
        ttk.Frame.__init__(self, parent)
        OptimizedGUI.__init__(self)
...
# ==================== 工厂信息采集 GUI ====================
class FactoryScraperGUI(ttk.Frame, OptimizedGUI):
    def __init__(self, parent):
        ttk.Frame.__init__(self, parent)
        OptimizedGUI.__init__(self)
        self.scraper = FactoryScraperCore()
        self.setup_callbacks()
        self.all_results = []
        self.is_scraping = False
        self.filter_keyword = ""
    ...
    def on_progress(self, current, total, current_id, items_count):
        self.after(0, lambda: self.update_progress(current, total, current_id, items_count))
    ...
    def create_control_panel(self):
        ...
        row1 = tk.Frame(control_frame, bg='#f0f0f0')
        row1.pack(fill=tk.X, padx=5, pady=5)
        tk.Label(row1, text="工厂类别数字:", bg='#f0f0f0', font=("微软雅黑", 10)).pack(side=tk.LEFT, padx=5)
        self.category_entry = tk.Entry(row1, width=15, font=("微软雅黑", 10))
        self.category_entry.pack(side=tk.LEFT, padx=5)
        self.category_entry.insert(0, "311")
        tk.Label(row1, text="采集页数(1-50):", bg='#f0f0f0', font=("微软雅黑", 10)).pack(side=tk.LEFT, padx=5)
        self.page_numbers = tk.Spinbox(row1, from_=1, to=50, width=8, font=("微软雅黑", 10))
        self.page_numbers.pack(side=tk.LEFT, padx=5)
        self.page_numbers.delete(0, 'end')
        self.page_numbers.insert(0, '10')
        tk.Label(row1, text="并发线程数:", bg='#f0f0f0', font=("微软雅黑", 10)).pack(side=tk.LEFT, padx=5)
        self.workers_spin = tk.Spinbox(row1, from_=1, to=5, width=6, font=("微软雅黑", 10))
        self.workers_spin.pack(side=tk.LEFT, padx=5)
        self.workers_spin.delete(0, 'end')
        self.workers_spin.insert(0, '2')
        tk.Label(row1, text="延时(秒):", bg='#f0f0f0', font=("微软雅黑", 10)).pack(side=tk.LEFT, padx=5)
        self.delay_spin = tk.Spinbox(row1, from_=0.2, to=2.0, increment=0.1, width=6, font=("微软雅黑", 10))
        self.delay_spin.pack(side=tk.LEFT, padx=5)
        self.delay_spin.delete(0, 'end')
        self.delay_spin.insert(0, '0.5')
    ...
    def create_table_view(self):
        filter_bar = tk.Frame(self.table_frame, bg='#f0f0f0', height=35)
        filter_bar.pack(fill=tk.X, padx=5, pady=5)
        tk.Label(filter_bar, text="公司筛选:", bg='#f0f0f0', font=("微软雅黑", 9)).pack(side=tk.LEFT, padx=5)
        self.filter_entry = tk.Entry(filter_bar, width=20, font=("微软雅黑", 9))
        self.filter_entry.pack(side=tk.LEFT, padx=5)
        self.filter_entry.bind("<Return>", lambda e: self.apply_filter())
        self.filter_apply_btn = tk.Button(filter_bar, text="应用筛选", command=self.apply_filter, bg='#4CAF50',
        ...
        scroll_x = ttk.Scrollbar(self.table_frame, orient=tk.HORIZONTAL)
        scroll_x.pack(side=tk.BOTTOM, fill=tk.X)
        columns = ( '公司名称', '成立年份', '年交易额', '月产值', '详细地址', '生产服务', '链接')
        self.tree = ttk.Treeview(self.table_frame, columns=columns, show='headings',
                                 yscrollcommand=scroll_y.set, xscrollcommand=scroll_x.set)
        self.tree.heading('公司名称', text='公司名称')
        self.tree.heading('成立年份', text='成立年份')
        self.tree.heading('年交易额', text='年交易额')
        self.tree.heading('月产值', text='月产值')
        self.tree.heading('详细地址', text='详细地址')
        self.tree.heading('生产服务', text='生产服务')
        self.tree.heading('链接', text='工厂链接')
        self.tree.column('公司名称', width=200)
        self.tree.column('成立年份', width=80)
        self.tree.column('年交易额', width=120)
        self.tree.column('月产值', width=120)
        self.tree.column('详细地址', width=250)
        self.tree.column('生产服务', width=150)
        self.tree.column('链接', width=300)
        self.tree.pack(fill=tk.BOTH, expand=True)
        scroll_y.config(command=self.tree.yview)
        scroll_x.config(command=self.tree.xview)
    ...
    def update_progress(self, current, total, current_id, items_count):
        if total <= 0:
            return
        percent = (current / total) * 100
        self._update_progress_with_throttle(current, total, percent)

        if total <= 50:  # 第一阶段：获取 memberId（页数 ≤50）
            self.progress_info_label.config(text=f"获取 memberId: {current} / {total} 页")
            self.progress_detail_label.config(text=f"当前页: {current}")
            self.progress_count_label.config(text=f"已获取: {items_count} 个会员")
        else:  # 第二阶段：获取工厂详情
            self.progress_info_label.config(text=f"获取工厂详情: {current} / {total}")
            self.progress_detail_label.config(text=f"当前ID: {current_id}")
            self.progress_count_label.config(text=f"已采集: {items_count} 条")
            if current == 1:
                self.log_message(f"第二阶段开始，共需采集 {total} 个工厂详情，请耐心等待...", 'info')
    ...
    def _add_item_to_tree(self, data):
        self.tree.insert('', tk.END, values=(
            ...
            data.get('companyYearStarted', ''),
            data.get('annualTradeVolume', ''),
            data.get('monthProductValue', ''),
            data.get('factoryDetailedAddress', ''),
            data.get('productionService', ''),
            data.get('url', '')
        ))
    ...
    def apply_filter(self):
        if self.is_scraping:
            self.log_message("采集进行中，请稍后...", 'warning')
            return
        self.filter_keyword = self.filter_entry.get().strip().lower()
        self.refresh_display()
    ...
    def refresh_display(self):
        if not self.filter_keyword:
            ...
            self.filtered_results = [item for item in self.all_results
                                     if self.filter_keyword in item.get('companyName', '').lower()
                                     ]
        self.clear_table()
        for data in self.filtered_results:
            self._add_item_to_tree(data)
        ...
        matched = len(self.filtered_results)
        if self.filter_keyword:
            self.filter_match_label.config(text=f"匹配 {matched} / {total} 条")
        else:
            self.filter_match_label.config(text=f"共 {total} 条数据")
    ...
    def on_tree_double_click(self, event):
        selection = self.tree.selection()
        if selection:
            url = self.tree.item(selection[0])['values'][7]
            if url:
                import webbrowser
                webbrowser.open(url)
    ...
    def start_scraping(self):
        ...
        try:
            ...
            messagebox.showerror("错误", "页数/并发数/延时无效")
            return
        ...
        if messagebox.askyesno("确认", "清空之前的数据吗？"):
            self.clear_data()
        self.is_scraping = True
        Thread(target=self.scraper.scrape, args=(category, pages, workers, delay), daemon=True).start()
    ...
    def save_data(self):
        ...
        filename = filedialog.asksaveasfilename(defaultextension=".json",
                                                filetypes=[("JSON文件", "*.json"), ("CSV文件", "*.csv")],
                                                initialfile=f"工厂信息_{self.category_entry.get()}_{time.strftime('%Y%m%d_%H%M%S')}")
        if filename:
            try:
                if filename.lower().endswith('.csv'):
                    self.save_as_csv(self.all_results, filename)
                else:
                    self.save_as_json(self.all_results, filename)
                self.log_message(f"保存成功: {filename}", 'info')
                messagebox.showinfo("成功", f"数据已保存至:\n{filename}")
            except Exception as e:
                self.log_message(f"保存失败: {str(e)}", 'error')
    ...
    def save_as_csv(self, data, filename):
        if not data:
            return
        fieldnames = ['companyName', 'companyYearStarted', 'annualTradeVolume',
                      'monthProductValue', 'factoryDetailedAddress', 'productionService', 'url']
        import csv
...
class MainApplication(tk.Tk):
    def __init__(self):
        ...
        self.factory_tab = FactoryScraperGUI(self.notebook)
        ...
        self.notebook.add(self.factory_tab, text="工厂信息采集")
